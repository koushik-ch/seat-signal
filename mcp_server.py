#!/usr/bin/env python3
"""
SeatSignal MCP server. Thin stdio/Streamable HTTP transport over core.py.
...
see what is actually bookable. Use pvr_recommend_seats when the user wants
specific seat labels to click, not just the size of the best free block.
pvr_now_showing answers "what's on".
...
- pvr_recommend_seats enumerates exact party-sized windows inside free runs and
  ranks them using a deterministic breakdown: horizontal centre, viewing
  depth, edge clearance, and preferred-zone membership. It returns the top
  pick, alternatives, scores, and human-readable reasons. The score is a
  recommendation, not a guarantee: seats can be taken between reading the map
  and completing the official booking flow.
...
mcp = FastMCP(
    ...
    icons=_icon(),
    website_url="https://github.com/koushik-ch/seatsignal",
    ...
    for film, rows in by_film.items():
        ...
        for name, lo, hi in _SLOTS:
            ...
            if not slot:
                continue
            out += ["", "### %s" % name, "",
                    "| Time | Screen | Format | Good seats free | Best block | Pick |",
                    "| --- | --- | --- | --- | --- | --- |"]
            for r in sorted(slot, key=lambda r: _hour24(r.get("time")) or 0):
                pick = r.get("recommendation") or {}
                pick_text = "%s (%.2f)" % (pick.get("span", "-"), pick.get("score", 0.0)) if pick else "-"
...
def pvr_recommend_seats(
    ...
    """Return exact seat labels ranked for each matching show.

    `pvr_seats` is the broad diagnostic view. This tool is the actionable
    follow-up: it returns the precise party-sized groups a user can click,
    their 0..1 score breakdown, and lower-ranked outside-zone alternatives.
    It deliberately reuses pvr_seats' one seat-map request per show rather than
    adding another upstream call.
    """
    if party_size < 1:
        return _err("PARTY_SIZE_REQUIRED", "party_size must be 1 or more.")
    if limit < 1:
        return _err("BAD_LIMIT", "limit must be 1 or more.")
    ...
    try:
        ...
        return _err("RECOMMENDATION_ERROR", "The seat lookup returned invalid data.")

    # pvr_seats' JSON contract is a list of report rows (rather than an object
    # wrapper). Keep this tool tolerant of either shape so future response
    # metadata can be added without breaking the recommendation surface.
    reports = payload if isinstance(payload, list) else payload.get("results") or []
    rows = []
    lines = []
    for report in reports:
        top = report.get("recommendation")
        alternatives = report.get("recommendations") or []
        outside = report.get("exact_alternatives") or []
        if top:
            rows.append(
                {
                    ...
                    "recommendation": top,
                    "alternatives": alternatives[: max(0, limit - 1)],
                    "outside_zone_alternatives": outside[:limit],
                    "zone_rows": report.get("zone_rows", []),
                    "widened_to": report.get("widened_to", []),
                }
            )
            lines.append(
                "%s  %s  %s  %s (%.2f)"
                % (
                    report.get("time", ""),
                    report.get("screen") or "-",
                    report.get("experience") or "-",
                    top.get("span", ""),
                    top.get("score", 0.0),
                )
            )
            lines.append("   %s" % "; ".join(top.get("reasons") or []))
            if report.get("widened_to"):
                lines.append("   preferred zone widened into %s" % ",".join(report["widened_to"]))
            if report.get("booking_url"):
                lines.append("   book: %s" % report["booking_url"])

    if not rows:
        return _err(
            "NO_RECOMMENDATION",
            "No matching show has enough adjacent free seats for this party.",
            matching_shows=len(reports),
        )
    ...
    header = "%s - cinema %s - %s\n%-9s %-8s %-8s %s\n%s" % (
        ...
        "SCREEN",
        "FORMAT",
        "RECOMMENDED SEATS (SCORE)",
        "-" * 78,
    )
    return _out({"recommendations": rows}, header + "\n" + "\n".join(lines), format)
...
def pvr_list_watches() -> str:
    ...
    for w in watches:
        ...
        if w.get("priority") is not None:
            controls.append("priority %s" % w.get("priority"))
        if w.get("quiet_hours"):
            controls.append("quiet %s" % w.get("quiet_hours"))
        if w.get("dedup_minutes") is not None:
            controls.append("dedup %sm" % w.get("dedup_minutes"))
        if controls:
            lines.append("  notification controls: %s" % ", ".join(controls))
    if dirty:
        lines.append(
...
def pvr_add_watch(
    ...
    priority is 1..5 (5 is urgent). quiet_hours accepts "23:00-07:00" and
    defers ordinary alerts until the window ends. dedup_minutes suppresses a
    repeated logical event for that many minutes; failed deliveries are never
    marked as delivered.
    ...
    try:
        watch_priority_value = max(1, min(5, int(priority)))
        watch_dedup_minutes = max(0, int(dedup_minutes))
    except (TypeError, ValueError):
        return "priority must be an integer from 1 to 5 and dedup_minutes must be a non-negative integer."
    ...
    watch = {
        ...
        "priority": watch_priority_value,
        "dedup_minutes": watch_dedup_minutes,
    }
    if experience:
    ...
    if zone_seats:
        try:
            ...
            return "zone_seats should look like '11-21'."
    if quiet_hours:
        parsed_quiet = quiet_hours.replace(",", "-").split("-", 1)
        if len(parsed_quiet) != 2:
            return "quiet_hours should look like '23:00-07:00'."
        try:
            start = datetime.datetime.strptime(parsed_quiet[0].strip(), "%H:%M")
            end = datetime.datetime.strptime(parsed_quiet[1].strip(), "%H:%M")
        except ValueError:
            return "quiet_hours should use 24-hour HH:MM values, e.g. '23:00-07:00'."
        watch["quiet_hours"] = [start.strftime("%H:%M"), end.strftime("%H:%M")]
